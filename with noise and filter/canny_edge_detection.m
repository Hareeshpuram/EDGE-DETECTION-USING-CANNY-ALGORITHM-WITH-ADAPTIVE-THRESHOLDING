function [J,In,fi,np ]= canny_edge_detection(I, sigma,Tl,Th,l)
...
ns=(np/100)*maxv;
In=imnoise(I,'gaussian',0,(ns^2)/(maxv^2));

%noise_density = 0.1; % noise density in percentage
%In = imnoise(I, 'salt & pepper', noise_density);
% horizontalnih i vertikalnih gradijenata
[I_dx, I_dy] = DRoG(In,sigma);
fi=imgaussfilt(In, 0.5);
...
% filtriranje (primena operatora)
I_dx = imfilter(I, H, 'replicate', 'same');
I_dy = imfilter(I, H', 'replicate', 'same'); 
%}
